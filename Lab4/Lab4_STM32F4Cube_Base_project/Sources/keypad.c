...
osThreadDef(Thread_KEYPAD, osPriorityNormal, 1, 0);		// thread definition struct (last argument default stack size)
...
static int interpret_key(int row, int col)
{
	...
	switch(values[row][col])
	{
		...
		case ('*'):
			return STAR_KEY; /* Just pick a value different than 0 to 9 */ 
		default:
...
{
	osSignalSet(tid_Thread_KEYPAD, KEYPAD_SIGNAL);
}
...
{
  tid_Thread_KEYPAD = osThreadCreate(osThread(Thread_KEYPAD), NULL); // Start SEGMENT Thread
  if (!tid_Thread_KEYPAD) return(-1); 
  return(0);
}

 /*----------------------------------------------------------------------------
*      Thread  'Thread_KEYPAD': Scans Keypad
 *---------------------------------------------------------------------------*/
void Thread_KEYPAD (void const *argument) 
{
	int counter = 0; 	/* Used for counting consecutive events related to same key press for debouncing */
	
	while(1)
	{
		osSignalWait(KEYPAD_SIGNAL, osWaitForever);
		printf("%d\n", counter);
		...
		{		
			switch (KEYPAD_STATE) {
				...
				case DEFINITELY_PRESSED:
					...
					switch (Keypad_ScanKey()) {
						case 1:
							Accelerometer_SetAngleMode(PITCH_MODE);
							break;
						case 2:
							Accelerometer_SetAngleMode(ROLL_MODE);
							break;
						case STAR_KEY:
							SevenSegment_ToggleDisplayMode();
							break;
						default:
							/* invalid key */
							break;
